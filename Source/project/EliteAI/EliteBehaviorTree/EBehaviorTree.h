...
#pragma region COMPOSITES
	...
	//--- PARALLEL --
	class BehaviorParallel : public BehaviorComposite
	{
	public:
		explicit BehaviorParallel(std::vector<IBehavior*> childrenBehaviors) :
			BehaviorComposite(childrenBehaviors) {}
		virtual ~BehaviorParallel() = default;
...
#pragma endregion
	...
	//-----------------------------------------------------------------
	// BEHAVIOR TREE INVERTER (IBehavior)
	//-----------------------------------------------------------------
	class BehaviorInverter : public IBehavior
	{
	public:
		explicit BehaviorInverter(std::function<bool(Blackboard*)> fp) : m_fpInverter(fp){}
		virtual BehaviorState Execute(Blackboard* pBlackBoard) override;

	private:
		std::function<bool(Blackboard*)> m_fpInverter = nullptr;
	};
